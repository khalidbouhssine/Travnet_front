...
import './css/DashboardRoomAddOne.css';
...
function DashboardAccount() {
  ...
  const addRoom=async()=>{
    if(RoomNumber!="" && Price!="" && Surface!="" && AirConditioner!="" && Bathroom!="" && Floor!="" && Description!=""){
      try {
        const response = await fetch('https://127.0.0.1:8000/addchambre', {
          method: 'POST',
          headers: {
        ...
        <div className="rightpage">
          <div className="pageDashboardAccountModifCentre">
            <div className="inpBox">
                <div className="placeTitle">ADD ROOM</div>
                <div className="inpPlace">
                    <div className="placeText">Room Number</div>
                    <div className="placeInput">
                        <input type="text" className="inp" placeholder="Room Number" value={RoomNumber} onChange={(e)=>{setRoomNumber(e.target.value)}}/>
                    </div>
                </div>
                <div className="inpPlace">
                ...
                <div className="btnmarginNone" onClick={addRoom}>Add Room</div>
                <Link to="/dashboardroomsadd" className="btnAddalot">ADD ROOMS</Link>
            </div>
          </div>
        </div>
      </div>
